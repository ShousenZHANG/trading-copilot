...
2. Analysts (PARALLEL — fan out all four in one Agent dispatch)
   ...
   - Wait for all four reports before starting the debate. If parallel dispatch hits rate limits, fall back to serial and log the fallback to `_errors.md`.
...
7. portfolio-manager (Opus)
   ...
   - Does not write memory or final report directly
   - Orchestrator validates the run, appends memory through scripts/memory.py, and assembles the report through scripts/assemble_report.py
...
Validation gates:
- `scripts/validate_outputs.py run data/runs/<TICKER>-<YYYY-MM-DD>` must pass before memory append or final report assembly.
- `scripts/memory.py append` is the only supported writer for `data/memory/trading_memory.md`.
- `scripts/assemble_report.py` is the only supported writer for assembled `/analyze` decision reports.
```
...
Single source of truth: [.claude/config/output-language.md](../../config/output-language.md). Edit that file to switch project-wide language.

- **Internal debate** (Bull/Bear/Risk debators) stays in **English** for reasoning quality. Non-negotiable.
- **User-facing output** (analyst reports, Research Manager rationale, Trader reasoning, Portfolio Manager thesis): follows `output-language.md` (current: **Chinese 中文**).
- **Always preserve** ticker symbols, indicator names (RSI, MACD, ATR), price numbers, FRED series IDs in English everywhere.
